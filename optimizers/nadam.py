...
class NAdam:
    ...
    def update(self, model):
        """
        Update the model parameters using NAdam
        ...
        for i in range(len(weights)):
            ...
            weights[i] -= self.learning_rate * (self.beta1 * m_hat_w + (1 - self.beta1) * dw[i] / (1 - self.beta1 ** self.t)) / (np.sqrt(v_hat_w) + self.epsilon)
            biases[i] -= self.learning_rate * (self.beta1 * m_hat_b + (1 - self.beta1) * db[i] / (1 - self.beta1 ** self.t)) / (np.sqrt(v_hat_b) + self.epsilon)
