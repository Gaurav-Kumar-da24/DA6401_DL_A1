...
def train_function():
    ...
    run.name = f"hl_{wandb.config.num_layers}_bs_{wandb.config.batch_size}_ac_{wandb.config.activation}_{wandb.config.optimizer}_MSE_loss"
    ...
    loss_type = getattr(config, 'loss', 'mean_squared_error')
